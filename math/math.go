...
func lesEcpe(a [][]float64, b []float64) (float64List []float64, err error) {
	/*
		线性代数方程组的列主元消去法
		输入   :
		    a       a x = b线性代数方程组的系数矩阵
	...
	*/
	...
	// 输入判断
	if len(atemp) != n {
		return sol, errors.Wrap(err, "Function lesEcpe() math error.")
	}
...
func Gaos() (err error) {
	...
	mx, err := lesEcpe(ma, mb)
	if mx != nil {
		return errors.Wrap(err, "Function Gaos math error.")
	}
	return err
}
